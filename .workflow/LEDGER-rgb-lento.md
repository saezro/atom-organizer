...
- [ ] Manifiesto SQLite: hoy 2 commits por imagen (`manifiesto.py:128-144`), sin batching. Revisar además seguridad de hilos de la conexión (el apply paralelo la usa desde varios hilos)
...
- [ ] Medición antes/después que demuestre la mejora
- [ ] Release y aviso a Rodrigo para validar en el PC de oficina
...
- [ ] Aplicar hallazgos de la revisión completa del motor nuevo (auditorías en curso: indice.py / apply.py / manifiesto+cierre+paralelismo)
...
- [ ] (original) 🔴 `apply.py:351-358` — `submit`/`marcar_en_curso` sin try/except: un `BrokenProcessPool` (worker muerto por RAM) aborta el lote entero y deja la fila 'en_curso' sin liberar aforo.
- [x] 🟠 térmicas — `resultado`/`completadas`/`convertidas` bajo `lock_contadores`; `resultado` de RGB bajo `lock_resultado`. **APLICADO**.
- [ ] (original) 🟠 `apply.py:536-554` (térmicas) — contadores `resultado["fallido"]`/`completadas["n"]` mutados sin lock desde varios hilos.
...
- [ ] (original) 🟠 `apply.py:330,349,574` — `progress_bar.emit` por CADA imagen, sin throttling (las stats sí lo tienen).
...
- [ ] (original) 🔴 `paralelismo.py:104-112,173` — `import psutil` sin `try/except`: si falta en el .exe congelado, tumba la fase entera a mitad de run. `utils.py:1289-1302` ya tiene el patrón defensivo a copiar.
...
- [ ] (original) 🔴 `cierre.py:205-220` — `verificar()` hace `existe_ruta`+`tamano_de` por fichero, secuencial (2 round-trips por ruta, hasta 3 rutas por fila). En GCS/SMB puede tardar más que el propio apply. Paralelizar con ThreadPoolExecutor y/o fusionar en una sola lectura de metadata.
...
- [ ] (original) 🟠 `cierre.py:44,92,190,222` — el manifiesto se recorre 2-3 veces enteras + consultas por vuelo repetidas.
...
- [ ] Release nueva con estos fixes (requiere OK de Rodrigo).
...
- [ ] Medir en Windows con las nuevas trazas (bloqueado: requiere release + PC de oficina)
...
- [ ] 🟢 El `_CROP` rotado ahora conserva EXIF (el viejo lo perdía, `pipeline.py:846`). Mejora, sin test que la cubra
- [ ] 🟢 Recorte y compresión: mismo código reutilizado de `pipeline.py`, criterio idéntico. El nuevo hace 1 decode en vez de 2-3 (mejor calidad a igual `quality=`)
